...
import { CartLine, ChannelDeliveryOptionConfiguration, ChargeLine } from '@msdyn365-commerce/retail-proxy/dist/Entities/CommerceTypes.g';
...
function _renderSubTotal(props: ICheckoutViewProps): JSX.Element | null {
    const OrderSubTotal = props.data.checkout.result?.checkoutCart.cart.SubtotalSalesAmount;
    return (
...
const getDeliveryMode = (
    ...
    return channelDeliveryOptionConfig?.PickupDeliveryModeCodes?.find((dm: string | undefined) => dm === deliveryMode);
};
...
function _renderShipping(props: ICheckoutViewProps): JSX.Element | null {
    ...
    const headerChargelines = props.data.checkout.result?.checkoutCart.cart.ChargeLines || [];
    let freightFee = 0;
    ...
    if (headerChargelines && headerChargelines.length > 0) {
        const shippingChargeLines = (headerChargelines || []).filter(chargeLine => chargeLine.IsShipping);
        shippingChargeLines &&
                ...
                if (chargeLines[0]) {
                    ...
                    freightFee =
                        ...
                        shippingChargeLines.reduce((chargeTotal, chargeLine) => {
                            return chargeTotal + (chargeLine.NetAmountWithAllInclusiveTax || 0);
                        }, 0);
                }
            });
    }
    return (
        <p className={`msc-order-summary__line-shipping`}>
            ...
            {isShippingCalculated && freightFee ? (
                ...
                <span className='msc-order-summary__value'>{props.resources.toBeCalculatedText}</span>
            )}
        </p>
    );
}

const OrderSummaryComponent: React.FC<ICheckoutViewProps> = (props: ICheckoutViewProps) => {
    ...
    const retailMultiplePickUpOptionEnabled = props.data.featureState?.result?.find(item => item.Name === multiplePickupStoreSwitchName)
        ?.IsEnabled;
    const deliveryModes = cartLines.map((cartLine: CartLine) => cartLine.DeliveryMode);
