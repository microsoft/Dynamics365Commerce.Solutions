...
namespace Contoso
{
    namespace CommerceRuntime.DocumentProvider.EpsonFP90IIISample.DocumentBuilders
    {
        ...
        /// <summary>
        /// RT2 version builder for building printRecItemOrSubtotalAdjustment element for the fiscal receipt document.
        /// </summary>
        public class DocumentElementAdjustmentBuilderRT2 : IDocumentElementAdjustmentBuilder
        {
            /// <summary>
            ...
            public XElement BuildPrintRecItemOrSubtotalAdjustment(SalesLine salesLine, XElement rootElement, string description, string department, bool isSalesLine, string operatorId, string justification)
            {
                ...
                var isSalesLineFullyDiscounted = salesLine.Price > 0 && salesLine.NetAmount == 0;
                ...
                if (salesLine.DiscountAmount != 0)
                {
                    ...
                    if (isSalesLineFullyDiscounted)
                    {
                        printRecItemAdjustmentElement.Add(new XAttribute(DocumentAttributeConstants.AdjustmentType, DocumentAttributeConstants.PrintRecItemOmaggioAdjustmentTypeValue));
                        printRecItemAdjustmentElement.Add(new XAttribute(DocumentAttributeConstants.VatId, department));
                    }
                    else
                    {
